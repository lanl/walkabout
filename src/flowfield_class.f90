...
character(len=70) :: controlfile, finfile , avsfile , zonefile 
 ...
 do 
  ...
  if( ios .ne. 0 ) then 
   ...
   ! line contains a file name and a zone name 
    ...
    do 
     read(unit=funit2, fmt=*, end=995) bzone  
     if( bzone .eq. azone) exit 
    end do 
...
!  vel=vrecon( nrmls, fluxes, ptr2cell)  
  ...
  disp(i,:,:)  = calculateBtensor(ffobj%tensor(i), vel) 
  dcoef(i,:,:) = calculateDtensor(ffobj%tensor(i), vel) 
  
  ! local time step limits  
  absvel= sqrt( dot_product(vel, vel) ) + small  

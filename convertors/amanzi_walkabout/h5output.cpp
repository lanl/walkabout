// modified h5output.cpp for new hdf5 libs
...
//the main function.  Controls the flow of the program by calling functions in the necessary order
int main(int argc, char* argv[]) {
	...
	if(param.pre){
		cout << "Running preprocessing steps!\n";
		//create a bunch of output files needed to run LaGriT and Walkabout
		writePly(data);
		writeInp(data);
		writeAvs(data);
		writeLgi(data);
...
dataStruct getData(char *filename){
	...
	//read in the walkabout data.  This data is cell centered + data on boundaries
...
//writeInp writes a .inp file, which is an input file for LaGriT specifying cell 
//centers and edge points for the domain
void writeInp(dataStruct data){
	ofstream myfile;
//	string filename = data.filename;
	string filename = "walkabout.h5.inp";
	myfile.open (filename.c_str());
	
	//write out header information
	myfile << data.size << " " << "0000000000 0000000000 0000000000 0000000000\n";
...
//pressure, saturation, porosity, and density information
void writeAvs(dataStruct data){
	ofstream myfile;
//	string filename = data.filename;
	...
	//write out header information
	myfile << "03  1   1   1   1\n";
	myfile << "Liquid Pressure (MPa), (MPa)\n";
	myfile << "Saturation, (no dim)\n";
	myfile << "Porosity, (no dim)\n";
//	myfile << "Liquid Density (kg/m**3), (kg/m**3)\n";
...
//writeLgi writes an LGI file, which is a control file for LaGriT.  It specifies input and output settings
//and controls what operations LaGritT should perform.
void writeLgi(dataStruct data){
	ofstream myfile;
	string filename = data.filename;
	filename += ".lgi";
...
void writeControl(dataStruct data){
	...
	if(!ifile){
		...
		cout << "Writing default control file.  Please modify control.dat to suit your needs.\n";
		...
		ifile.close();
		cout << "Not writing control file because one already exists.\n";
	}
}
...
//executables that can be run as part of the process that this program goes through.
parameterStruct readConfig(){
